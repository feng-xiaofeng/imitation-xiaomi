...
func (con GoodsTypeAttributeController) Index(c *gin.Context) {
	...
	//获取商品类型属性
	goodsTypeAttributeList := []goods.GoodsTypeAttribute{}
...
func (con GoodsTypeAttributeController) DoAdd(c *gin.Context) {
	...
	goodsTypeAttr := goods.GoodsTypeAttribute{
		Title:     title,
		CateId:    cateId,
		AttrType:  attrType,
		AttrValue: attrValue,
...
func (con GoodsTypeAttributeController) Edit(c *gin.Context) {
	...
	// 获取当前商品id类型属性
	goodsTypeAttribute := goods.GoodsTypeAttribute{Id: id}
...
func (con GoodsTypeAttributeController) DoEdit(c *gin.Context) {
	...
	if err4 != nil {
		con.Error(c, "排序必须是整数", "/admin/goodsType/edit?id="+models.String(id))
		return
	}
	goodsTypeAttr := goods.GoodsTypeAttribute{Id: id}
...
func (con GoodsTypeAttributeController) Delete(c *gin.Context) {
	...
	fmt.Println(cateId)
	if err1 != nil || err2 != nil {
		con.Error(c, "传入参数错误", "/admin/goodsTypeAttribute?id="+models.String(id))
		return
	}
	goodsTypeAttribute := goods.GoodsTypeAttribute{Id: id}
